...
assert(/preserveLocalBrowserSessions\(remoteState\);\s*(preserveLocalHallTableLocks\(remoteState\);\s*)?migrateDisplayState\(remoteState\);/.test(app), 'Remote state must preserve sessions before migration/session validation, allowing local table-lock preservation before migration.');
