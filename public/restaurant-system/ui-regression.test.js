...
const staffLoginRouteSource = fs.readFileSync(path.join(root, '..', '..', 'app', 'api', 'staff-login', 'route.ts'), 'utf8');
...
mustContain(html, 'styles.css?v=cashier-yellow-lock-canonical-customer-163');
mustContain(html, 'core.js?v=cashier-yellow-lock-canonical-customer-163');
mustContain(html, 'app.js?v=cashier-yellow-lock-canonical-customer-163');
mustContain(html, 'core.js?v=cashier-yellow-lock-canonical-customer-163');
...
mustContain(app, 'cashier-yellow-lock-canonical-customer-163', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'برای بستن صندوق، اول همه میزها و سفارش‌های تسویه‌نشده را تسویه کنید.', 'با سفارش ثبت‌شده باز/تسویه‌نشده نباید اجازه بستن صندوق داده شود.');
...
mustContain(coreSource, "throw new Error('OPEN_HALL_TABLES_EXIST')", 'core باید اجازه بستن صندوق با سفارش ثبت‌شده باز/تسویه‌نشده را ندهد.');
assert(!coreSource.includes("throw new Error('HALL_TABLE_LOCKS_EXIST')"), 'میز زرد/در حال ثبت سفارش نباید مانع بستن صندوق شود؛ فقط سفارش ثبت‌شده باز مانع است.');
mustContain(coreSource, 'state.hallTableLocks = state.hallTableLocks.filter((lock) => lock.customerId !== customerId);', 'بستن صندوق باید قفل‌های زرد همان رستوران را هم پاک کند تا میزها گیر نکنند.');
...
mustContain(restaurantStateApiSource, 'customerId: manager.customerId', 'اکانت مدیر باید customerId همان رستوران را به iframe بدهد تا lock میز با صندوق‌دار مشترک شود.');
mustContain(managerSessionSource, "customerId: String(customer?.id || staff.customerId || '')", 'manager session باید customer canonical همان tenant را مقدم کند نه customerId stale پرسنل را.');
mustContain(restaurantStateApiSource, 'customerId: staff.customerId', 'اکانت صندوقدار باید customerId همان رستوران را به iframe بدهد تا lock مدیر را ببیند.');
mustContain(restaurantStateApiSource, 'function canonicalPortalCustomer(sharedState: any, tenantId: string, identity: any = {})', 'API باید customerId canonical را از state سرور برای owner/manager/staff برگرداند.');
mustContain(restaurantStateApiSource, 'const responseIdentity = canonicalPortalCustomer(data?.state, tenant.id, identity);', 'GET restaurant-state باید identity را با customerId واقعی tenant کامل کند.');
mustContain(restaurantStateApiSource, 'const responseIdentity = canonicalPortalCustomer(sharedState, tenant.id, identity);', 'PUT restaurant-state هم باید identity برگشتی را با customerId واقعی tenant کامل کند.');
mustContain(app, "customerId: String(meta.customerId || '')", 'portal identity باید customerId را نگه دارد.');
...
mustContain(app, "const onlinePortalHost = /(^|\\.)flowkave\\.tech$/i.test(window.location.hostname);", 'روی دامنه آنلاین flowkave.tech سامانه رستوران مستقیم هم باید portal sync شود نه local /api/state.');
mustContain(app, "const portalMode = !publicQrMode && (portalParams.get('portal') === '1' || onlinePortalHost);", 'portalMode آنلاین نباید به وجود ?portal=1 وابسته باشد و QR عمومی باید جدا بماند.');
mustContain(app, "if (portalMode) return ensurePortalCustomer(portalIdentity);", 'قفل زرد میز در portal باید حتی بدون session محلی معتبر از customer canonical سرور استفاده کند.');
mustContain(app, "const lockCustomer = currentCustomer() || customer;", 'انتخاب میز باید lock را با customer canonical فعلی بسازد نه customer stale رندر قبلی.');
mustContain(app, "acquireHallTableLock(lockCustomer.id, tableId)", 'hallTableLocks باید با customer canonical ذخیره شود تا دستگاه‌های دیگر همان قفل زرد را ببینند.');
mustContain(app, "publicQrMode ? `/api/public-restaurant-state?tenantId=${encodeURIComponent(publicTenantId)}` : '/api/state'", 'QR عمومی publicTenant باید همچنان از endpoint عمومی استفاده کند و local/dev fallback خراب نشود.');
...
mustContain(app, "if (!acquireHallTableLock(lockCustomer.id, tableId)) { delete hallOrderDrafts[tableId]; selectedHallTableId = '';", 'اگر قفل گرفتن میز شکست خورد draft خالی همان میز باید پاک شود.');
...
mustContain(app, 'cashier-yellow-lock-canonical-customer-163', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(app, "function isActiveRemoteHallOrderForTable(order, tableId)", 'sync نباید lock زرد local را روی سفارش فعال remote تزریق کند.');
mustContain(app, "return posStatus !== 'paid' || order.tableHeldAfterPayment === true;", 'paid-held هم باید active حساب شود تا سبز با lock زرد خراب نشود.');
mustContain(app, "const isDraftLocked = table.status === 'free' && Boolean(lock);", 'زرد فقط باید برای میز آزاد دارای lock باشد، نه میز دارای سفارش ثبت‌شده.');
mustContain(app, "const isSubmittedHallOrder = ['open-order','waiting-payment'].includes(table.status);", 'سفارش ثبت‌شده باید status مستقل قرمز/در انتظار پرداخت داشته باشد.');
mustContain(app, "const visualStatus = isPaidHeld ? 'paid-held' : (isSubmittedHallOrder ? table.status : (isDraftLocked ? 'draft-locked' : table.status));", 'اولویت رنگ میز باید سبز paid-held، بعد قرمز سفارش ثبت‌شده، و فقط بعدش زرد draft باشد.');
mustContain(app, "${visualStatus}", 'کلاس رنگ میزهای درگیر باید از visualStatus بیاید تا قرمز/سبز بر زرد اولویت داشته باشد.');
mustContain(app, "const blockedByOtherDraft = isDraftLocked && lockedByOther;", 'disabled شدن میز توسط دستگاه دیگر فقط باید برای draft زرد باشد، نه سفارش قرمز ثبت‌شده.');
...
mustContain(dashboardSource, 'cashier-yellow-lock-canonical-customer-163', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-yellow-lock-canonical-customer-163') && index.includes('core.js?v=cashier-yellow-lock-canonical-customer-163') && index.includes('app.js?v=cashier-yellow-lock-canonical-customer-163'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
