...
mustContain(html, 'styles.css?v=cashier-uniform-line-buttons-147');
mustContain(html, 'core.js?v=cashier-uniform-line-buttons-147');
mustContain(html, 'app.js?v=cashier-uniform-line-buttons-147');
mustContain(html, 'core.js?v=cashier-uniform-line-buttons-147');
...
mustContain(app, 'cashier-uniform-line-buttons-147', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, "const hallSubmitLabel = !currentShift ? 'اول صندوق را باز کنید' : (selectedTable ? 'ثبت سفارش' : 'اول میز را انتخاب کنید');", 'دکمه اصلی لاین میز باید متن کوتاه و ثابت ثبت سفارش داشته باشد تا اندازه دکمه‌ها یکسان بماند.');
...
mustContain(app, "selectedTable ? 'ثبت سفارش' : 'اول میز را انتخاب کنید'", 'دکمه ثبت سفارش جدید سالن باید متن کوتاه و ثابت «ثبت سفارش» داشته باشد.');
...
mustContain(app, 'cashier-uniform-line-buttons-147', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(dashboardSource, 'cashier-uniform-line-buttons-147', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-uniform-line-buttons-147') && index.includes('core.js?v=cashier-uniform-line-buttons-147') && index.includes('app.js?v=cashier-uniform-line-buttons-147'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
