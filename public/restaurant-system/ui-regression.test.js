...
mustContain(html, 'styles.css?v=remove-unused-payment-box-203');
mustContain(html, 'core.js?v=remove-unused-payment-box-203');
mustContain(html, 'app.js?v=remove-unused-payment-box-203');
mustContain(html, 'core.js?v=remove-unused-payment-box-203');
...
mustContain(app, 'function currentRegisterTurnoverSummary(customer)', 'کارت‌های بالای صفحه باید از صندوق روز کاری باز محاسبه شوند.');
mustContain(app, 'register-turnover-stats', 'کارت‌های بالای صفحه باید حالت ترنوور صندوق داشته باشند.');
mustContain(app, '<article><span>درآمد تقریبی</span><strong>${money(registerTurnover.revenue)}</strong></article>', 'باکس درآمد باید متن درآمد تقریبی داشته باشد و کپشن ترنوور صندوق باز نداشته باشد.');
mustContain(app, '<article><span>قیمت تمام‌شده</span><strong>${money(registerTurnover.cost)}</strong></article>', 'باکس قیمت تمام‌شده نباید کپشن ترنوور صندوق باز داشته باشد.');
mustContain(app, "${registerTurnover.shift ? '' : '<em>صندوق بسته است</em>'}", 'در سود تقریبی فقط وقتی صندوق بسته است متن صندوق بسته است نمایش داده شود.');
assert(!app.includes('<em>ترنوور صندوق باز</em>'), 'کپشن ترنوور صندوق باز باید از کارت‌های بالای صفحه حذف شود.');
assert(!app.includes("registerTurnover.profit >= 0 ? 'مثبت' : 'منفی'"), 'وقتی صندوق باز است زیر سود تقریبی نباید مثبت/منفی نوشته شود.');
mustContain(styles, 'Top register metric cards: larger centered text', 'CSS متن کارت‌های درآمد/قیمت تمام‌شده/سود باید بزرگ‌تر و وسط‌چین شود.');
mustContain(styles, '.register-turnover-stats article strong{', 'مبلغ کارت‌های بالای صفحه باید سایز بزرگ‌تر اختصاصی داشته باشد.');
...
mustContain(app, 'remove-unused-payment-box-203', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'function showHallPaymentMethodChoice(form)', 'ثبت پرداخت باید اول پاپ‌آپ انتخاب روش پرداخت را باز کند.');
mustContain(app, 'data-hall-payment-method-cash>نقدی</button>', 'در پاپ‌آپ روش پرداخت، دکمه نقدی باید فعال و ادامه‌دهنده پرداخت باشد.');
mustContain(app, 'disabled>کارت‌خوان</button>', 'کارت‌خوان فعلاً تا تعریف حسابداری نباید پرداخت را ثبت کند.');
mustContain(app, 'disabled>کارت به کارت</button>', 'کارت به کارت فعلاً تا تعریف حسابداری نباید پرداخت را ثبت کند.');
mustContain(app, "form.dataset.paymentMethodChoice = 'نقدی';", 'انتخاب نقدی باید روش پرداخت را برای submit همان فرم ثبت کند.');
mustContain(app, "if (id === 'hallPaymentForm' && !form.dataset.paymentMethodChoice) { showHallPaymentMethodChoice(form); return; }", 'submit پرداخت باید قبل از آزاد/درگیر، روش پرداخت را بخواهد.');
mustContain(app, "paymentMethod: form.dataset.paymentMethodChoice || f.get('paymentMethod') || 'نقدی'", 'ثبت واقعی پرداخت باید روش نقدی انتخاب‌شده را به core بدهد.');
mustContain(app, "if (!confirm('مطمئن هستید می‌خواهید این میز آزاد شود؟')) return;", 'آزاد کردن میز سبز باید قبل از release از کاربر تأیید بگیرد.');
mustContain(styles, 'Hall payment cash method choice before table release choice.', 'popup انتخاب روش پرداخت باید CSS کوچک و مستقل داشته باشد.');
mustContain(styles, 'Hall payment method/release choices must sit above the payment popup.', 'پاپ‌آپ روش پرداخت و آزاد/درگیر باید بالاتر از پاپ‌آپ پرداخت باشند نه زیر آن.');
mustContain(styles, '.hall-payment-method-choice-overlay,.hall-payment-release-choice-overlay{z-index:1400!important', 'z-index پاپ‌آپ‌های مرحله‌ای پرداخت باید از hall-payment-popup بالاتر باشد.');
...
mustContain(app, 'function ensurePublicQrTableLock(customerId, table)', 'باز شدن QR میز آزاد باید قبل از ثبت سفارش lock زرد مشترک بسازد.');
mustContain(app, 'if (table && !tableIsBlocked) ensurePublicQrTableLock(customerId, table);', 'QR فقط وقتی میز آزاد است lock موقت بسازد و مسیر سفارش/پرداخت را دست نزند.');
mustContain(app, 'publicQrOwnActiveTableLock(table)', 'lock خود همان موبایل نباید جلوی ثبت سفارش خودش را بگیرد.');
mustContain(app, 'const publicQrSubmittedTableLocks = new Set();', 'بعد از ثبت سفارش QR همان تب موبایل نباید بعد از پرداخت/آزادسازی دوباره همان میز را زرد کند.');
mustContain(app, 'publicQrSubmittedTableLocks.has(table.id)', 'QR pre-lock باید برای میزی که همین تب قبلاً ثبت کرده skip شود تا بعد از آزادسازی دوباره زرد برنگردد.');
mustContain(app, 'publicQrSubmittedTableLocks.add(table.id); releaseHallTableLock(customerId, table.id);', 'ثبت سفارش QR باید هم lock را release کند هم جلوی relock خودکار همان تب را بگیرد.');
mustContain(app, 'await flushSharedStateSave(localStorage.getItem(STORAGE_KEY) || JSON.stringify(state));', 'lock زرد QR باید فوری روی سرور flush شود تا صندوق‌ها ببینند.');
mustContain(app, "releaseHallTableLock(customerId, table.id);", 'بعد از ثبت سفارش QR باید lock زرد موقت آزاد شود تا سفارش ثبت‌شده قرمز شود.');
...
mustContain(app, 'const qrCards = tables.map((table) => {', 'QRهای چیدمان باید از tables فعلی ساخته شوند تا بعد از حذف میزها QR قدیمی نمایش داده نشود.');
...
mustContain(app, 'remove-unused-payment-box-203', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(publicRestaurantStateApiSource, "for (const key of ['shifts', 'ledger'])", 'endpoint عمومی باید فقط collectionهای افزایشی را merge-by-id کند و restaurantTables را جداگانه با تنظیمات چیدمان replace/filter کند.');
mustContain(publicRestaurantStateApiSource, 'function mergeRestaurantTables(existingTables: any[] = [], incomingTables: any[] = [], customers: any[] = [], orders: any[] = [])', 'endpoint عمومی باید حذف میزهای چیدمان را حفظ کند و میزهای حذف‌شده را از snapshot قدیمی برنگرداند.');
...
mustContain(coreSource, 'customer.hallTableSettingsUpdatedAt = new Date().toISOString();', 'تغییر چیدمان میز باید timestamp داشته باشد تا snapshot قدیمی دستگاه دیگر تنظیمات میز را برنگرداند.');
...
mustContain(app, 'function renderHallOrderNotePopup(customer)', 'یادداشت سفارش سالن باید در پاپ‌آپ مستقل رندر شود.');
mustContain(app, 'name="hallOrderNotePopupText"', 'پاپ‌آپ یادداشت باید textarea مستقل برای توضیحات، آلرژی و تغییرات داشته باشد.');
mustContain(app, 'function currentSelectedHallOrderNote(customer)', 'پاپ‌آپ یادداشت باید برای میز دارای سفارش باز، یادداشت خود سفارش را نشان دهد نه فقط draft قبل از ثبت.');
mustContain(app, 'activeOrder.orderNote = note;', 'ذخیره یادداشت بعد از ثبت سفارش باید روی خود سفارش باز نوشته شود.');
mustContain(app, "await persistCriticalState('ذخیره یادداشت سفارش روی سرور ناموفق بود؛ دوباره تلاش کنید.');", 'یادداشت سفارش ثبت‌شده باید روی سرور sync شود تا با برگشت به میز باقی بماند.');
mustContain(app, "const previewEl = hallPaymentFormLive.querySelector('[data-hall-payment-preview]'); if (previewEl)", 'حذف باکس مبلغ انتخاب‌شده نباید آپدیت زنده پرداخت را با null preview خراب کند.');
...
mustContain(app, "table.activeOrderId || (lock && lock.ownerId !== hallTableLockOwnerId())", 'QR میز باید سفارش باز یا lock صندوق/موبایل دیگر را ببندد ولی lock خود همان موبایل را برای ثبت سفارش مجاز بداند.');
...
mustContain(app, "let hallPaymentPopupOrderId = '';", 'پرداخت میز باز باید state popup جدا داشته باشد و منطق پرداخت تغییر نکند.');
mustContain(app, 'function renderHallPaymentOverlay(customer)', 'کلیک روی میز باز باید popup تقسیم فیش و پرداخت باز کند.');
...
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls{grid-template-columns:repeat(3,minmax(180px,1fr))', 'کنترل درصد، مبلغ تومان و حذف حق سرویس باید در popup عریض کنار هم قرار بگیرند.');
mustContain(app, 'class="hall-payment-popup-title"', 'اطلاعات اصلی پرداخت باید در header popup کنار دکمه بستن قرار بگیرد.');
mustContain(app, '<small>شماره سفارش</small><b>${receiptNumberText(order.trackingNumber || 0)}</b>', 'شماره سفارش در header popup باید دوخطی باشد.');
mustContain(app, '<small>مالیات</small><b>${money(order.taxTotal || 0)}</b>', 'مالیات در header popup باید دوخطی باشد.');
mustContain(app, '<small>مبلغ کل</small><b data-hall-summary-grand>${money(orderFinalTotal(order))}</b>', 'مبلغ کل در header popup باید دوخطی و قابل update باشد.');
mustContain(app, 'type="submit" form="hallPaymentForm"', 'دکمه ثبت پرداخت header باید همان فرم پرداخت قبلی را submit کند.');
mustContain(app, 'data-close-hall-payment-popup>افزودن آیتم</button>', 'دکمه افزودن به سفارش باید با متن کوتاه افزودن آیتم نمایش داده شود.');
assert(!app.includes("<div class=\"section-title\"><h2>تقسیم فیش و پرداخت ${esc(order.tableName || '')}</h2>"), 'عنوان داخلی تقسیم فیش و پرداخت نباید داخل کادر فرم باقی بماند.');
assert(!app.includes('<div class="hall-payment-summary"><div><span>شماره سفارش</span>'), 'summary هفت‌باکسی شماره/جمع/مالیات/حق سرویس/مبلغ/پرداخت/باقی‌مانده باید از داخل فرم حذف شود.');
assert(!app.includes("<button class=\"primary\" ${remaining.length ? '' : 'disabled'}>ثبت پرداخت</button>"), 'دکمه ثبت پرداخت نباید پایین فرم تکرار شود؛ فقط در header popup باشد.');
mustContain(styles, '.hall-payment-popup-title{display:flex!important', 'header popup پرداخت باید یک لاین flex داشته باشد.');
mustContain(styles, '.hall-payment-popup .hall-payment-panel>.section-title,.hall-payment-popup .hall-payment-panel>.hall-payment-summary{display:none!important}', 'اگر markup قدیمی برگشت، CSS popup نباید title/summary داخلی را نمایش دهد.');
mustContain(styles, 'Hall payment popup header compact stats and button parity.', 'ظاهر header popup پرداخت باید guard اختصاصی برای دوخطی شدن آمار و هم‌قد شدن دکمه‌ها داشته باشد.');
mustContain(styles, '.hall-payment-popup-meta .hall-payment-popup-stat{display:grid!important', 'شماره سفارش/مالیات/مبلغ کل باید دوخطی باشند تا اعداد بزرگ جا شوند.');
mustContain(styles, '.hall-payment-popup-meta>button,.hall-payment-popup .hall-service-charge-controls button{min-height:0!important;height:auto!important;padding:9px 14px!important;border:0!important', 'دکمه‌های افزودن آیتم و حذف حق سرویس باید مثل ثبت پرداخت هم‌قد و بدون کادر باشند.');
mustContain(app, 'const serviceCleared = order.serviceChargeDefaultCleared === true;', 'حذف دستی حق سرویس باید مانع برگشت خودکار پیش‌فرض ۵٪ شود.');
mustContain(app, 'const useDefaultService = !serviceCleared && !hasSavedService;', 'حق سرویس پیش‌فرض باید فقط برای فیش بدون تنظیم ذخیره‌شده فعال شود.');
mustContain(app, "const mode = useDefaultService ? 'percent' : (order.serviceChargeMode || '');", 'حق سرویس سفارش تازه باید به صورت پیش‌فرض روی محاسبه درصدی باشد.');
mustContain(app, 'Number(useDefaultService ? 5 : (order.serviceChargePercent || 0))', 'درصد پیش‌فرض حق سرویس باید ۵٪ باشد.');
mustContain(app, 'order.serviceChargeDefaultCleared = true;', 'وقتی کاربر حذف حق سرویس را می‌زند، پیش‌فرض ۵٪ نباید دوباره برگردد.');
mustContain(app, '<strong>حق سرویس</strong> — محاسبه درصدی', 'کلمه حق سرویس باید در همان لاین محاسبه درصدی ولی به صورت title بولد باشد.');
mustContain(styles, 'Hall payment popup centered variable-width stats and service label compaction.', 'باکس شماره سفارش/مالیات/مبلغ کل باید وسط‌چین و با عرض متغیر guard شود.');
mustContain(styles, 'justify-items:center!important;text-align:center!important;width:max-content!important;max-width:none!important', 'باکس‌های header باید وسط‌چین باشند و با طولانی شدن مقدار عریض شوند.');
mustContain(styles, 'Hall payment popup service spacing final tune.', 'فاصله خالی بین header popup و حق سرویس باید کم شود.');
mustContain(styles, '.hall-payment-popup-scroll{padding-top:8px!important}', 'scroll body popup نباید از بالا فاصله بی‌خودی داشته باشد.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls button{align-self:center!important}', 'دکمه حذف حق سرویس باید از بالا به پایین وسط‌چین باشد.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls label:first-child strong{font-weight:1000!important', 'عنوان حق سرویس باید بولدتر از محاسبه درصدی باشد.');
mustContain(styles, 'Hall payment popup header two-size font bump.', 'فونت header popup پرداخت باید دو سایز بزرگ‌تر شود.');
mustContain(styles, '.hall-payment-popup-meta .hall-payment-popup-stat small{font-size:13px!important}', 'عنوان‌های شماره سفارش/مالیات/مبلغ کل باید بزرگ‌تر باشند.');
mustContain(styles, '.hall-payment-popup-meta .hall-payment-popup-stat b{font-size:16px!important}', 'مقادیر شماره سفارش/مالیات/مبلغ کل باید بزرگ‌تر باشند.');
mustContain(styles, '.hall-payment-popup-meta>button{font-size:16px!important}', 'فونت دکمه‌های ثبت پرداخت و افزودن آیتم باید بزرگ‌تر باشد.');
mustContain(app, '<span>مبلغ</span><input type="radio" name="serviceMode" value="amount"', 'برچسب مبلغ حق سرویس باید مبلغ باشد نه مبلغ دستی.');
assert(!app.includes('<span>مبلغ دستی</span>'), 'عبارت مبلغ دستی نباید در لاین حق سرویس باقی بماند.');
mustContain(app, "const amountValue = mode === 'percent' ? Math.round(Number(order.subtotal ?? order.total ?? 0) * percentValue / 100)", 'در حالت درصدی باید مبلغ محاسبه‌شده درصد داخل باکس مبلغ نمایش داده شود.');
mustContain(app, "${mode === 'percent' ? 'readonly' : ''} aria-label=", 'وقتی حالت درصدی فعال است باکس مبلغ باید نمایشی باشد نه دستی.');
mustContain(app, "if (serviceAmountInput && updatedOrder.serviceChargeMode === 'percent')", 'با تغییر درصد، باکس مبلغ باید همان لحظه مبلغ محاسبه‌شده را نشان بدهد.');
mustContain(app, 'serviceAmountInput.readOnly = true;', 'در حالت درصدی input مبلغ باید readonly شود.');
mustContain(app, 'else if (serviceAmountInput) serviceAmountInput.readOnly = false;', 'در حالت مبلغ، input مبلغ باید دستی و قابل ویرایش باشد.');
mustContain(coreSource, 'function normalizeOrderDiscountInput(input = {})', 'تخفیف فیش سالن باید normalize محدود مثل حق سرویس داشته باشد.');
mustContain(coreSource, 'function setOrderDiscount(state, customerId, orderId, input = {})', 'تخفیف فیش سالن باید setter مستقل داشته باشد.');
mustContain(coreSource, 'setOrderDiscount,', 'setter تخفیف باید از RestaurantCore export شود.');
mustContain(app, 'function renderHallDiscountControls(order)', 'زیر لاین حق سرویس باید لاین تخفیف مستقل رندر شود.');
mustContain(app, 'data-hall-discount-box', 'لاین تخفیف باید داخل فرم پرداخت سالن وجود داشته باشد.');
mustContain(app, '<strong>تخفیف</strong> — محاسبه درصدی', 'لاین تخفیف باید مثل حق سرویس حالت درصدی داشته باشد.');
mustContain(app, 'name="discountPercent"', 'باکس درصد تخفیف باید موجود باشد و پیش‌فرض خالی بماند.');
mustContain(app, 'name="discountAmount"', 'باکس مبلغ تخفیف باید موجود باشد و پیش‌فرض خالی بماند.');
mustContain(app, 'data-clear-discount>حذف تخفیف</button>', 'لاین تخفیف باید دکمه حذف تخفیف داشته باشد.');
mustContain(app, '${renderHallServiceChargeControls(order)}${renderHallDiscountControls(order)}', 'لاین تخفیف باید دقیقاً زیر حق سرویس و قبل از اقلام باقی‌مانده باشد.');
mustContain(app, 'RestaurantCore.setOrderDiscount', 'فرم پرداخت باید تخفیف را با core setter اعمال کند.');
mustContain(app, "if (discountAmountInput && updatedOrder.discountMode === 'percent')", 'در حالت درصدی تخفیف، باکس مبلغ باید مبلغ محاسبه‌شده را نشان دهد.');
mustContain(app, "hallPaymentFormLive.querySelector('[data-clear-discount]')", 'دکمه حذف تخفیف باید درصد و مبلغ تخفیف را خالی کند.');
assert(!app.includes('useDefaultDiscount'), 'تخفیف نباید مقدار پیش‌فرض داشته باشد.');
mustContain(styles, 'Hall payment service/discount identical row lock.', 'لاین تخفیف باید از همان CSS مشترک و دقیقاً هم‌اندازه لاین حق سرویس استفاده کند.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls,.hall-payment-popup .hall-discount-controls{display:grid!important;grid-template-columns:minmax(330px,1fr) minmax(240px,.8fr) max-content!important', 'حق سرویس و تخفیف باید grid ستون‌بندی یکسان داشته باشند.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls>label,.hall-payment-popup .hall-discount-controls>label{display:grid!important;grid-template-columns:max-content auto minmax(76px,112px) max-content!important', 'labelهای حق سرویس و تخفیف باید اندازه داخلی یکسان داشته باشند.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls>button,.hall-payment-popup .hall-discount-controls>button{align-self:center!important;justify-self:start!important;width:max-content!important}', 'دکمه‌های حذف حق سرویس و حذف تخفیف باید جایگاه و اندازه یکسان داشته باشند.');
mustContain(styles, 'Hall payment radio size restore after discount row alignment.', 'radioهای مبلغ/درصد نباید با grid هم‌اندازه input مبلغ شوند.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls>label input[type="radio"],.hall-payment-popup .hall-discount-controls>label input[type="radio"]{width:16px!important;height:16px!important', 'دایره‌های radio در لاین حق سرویس و تخفیف باید اندازه طبیعی داشته باشند.');
mustContain(styles, 'min-width:16px!important;max-width:16px!important;inline-size:16px!important;block-size:16px!important', 'radio نباید به اندازه باکس مبلغ کشیده شود.');
assert(!app.includes('<h3>اقلام باقیمانده برای پرداخت</h3>'), 'عنوان اقلام باقیمانده برای پرداخت باید حذف شود.');
assert(!app.includes('<div class="hall-payment-history"><h3>پرداخت‌های انجام‌شده</h3>'), 'بخش پرداخت‌های انجام‌شده نباید داخل popup پرداخت نمایش داده شود.');
assert(!app.includes('<p>هنوز پرداختی ثبت نشده است.</p>'), 'پیام هنوز پرداختی ثبت نشده است نباید پایین popup پرداخت نمایش داده شود.');
mustContain(app, '<details class="hall-remaining-list hall-remaining-dropdown"${hallPaymentItemsDropdownOpen ? \' open\' : \'\'}>', 'آیتم‌های پرداخت باید داخل dropdown کشویی باشند.');
mustContain(app, '<summary class="hall-select-all"><label><input type="checkbox" data-hall-pay-all checked><span>همه آیتم ها برای تسویه کامل</span></label><b data-hall-items-subtotal>${money(remainingSubtotal)}</b></summary>', 'لاین انتخاب همه باید checkbox پیش‌فرض، متن جدید و مبلغ جمع آیتم‌ها داشته باشد.');
mustContain(app, 'const remainingSubtotal = remaining.reduce', 'جمع آیتم‌ها باید بدون مالیات/حق سرویس/تخفیف محاسبه شود.');
...
mustContain(app, '${orderNoteBox}</form>', 'باکس یادداشت باید انتهای فرم پرداخت و فقط به‌صورت شرطی رندر شود.');
assert(!app.slice(app.indexOf('function renderHallPaymentPanel'), app.indexOf('function collectSaleLines')).includes('مبلغ انتخاب‌شده:'), 'باکس مبلغ انتخاب‌شده/سهم تخفیف نباید در پاپ‌آپ پرداخت باقی بماند.');
assert(!app.slice(app.indexOf('function renderHallPaymentPanel'), app.indexOf('function collectSaleLines')).includes('<label>روش پرداخت'), 'روش پرداخت باید از پاپ‌آپ پرداخت حذف شود.');
assert(!app.slice(app.indexOf('function renderHallPaymentPanel'), app.indexOf('function collectSaleLines')).includes('name="paymentMethod"'), 'select روش پرداخت نباید در فرم پرداخت سالن رندر شود.');
assert(!app.slice(app.indexOf('function renderHallPaymentPanel'), app.indexOf('function collectSaleLines')).includes('یادداشت سفارش'), 'روی باکس یادداشت داخل پرداخت نباید تیتر/label یادداشت سفارش نوشته شود.');
mustContain(styles, 'Hall payment order note only when present; replaces selected amount preview/payment method UI.', 'باکس شرطی یادداشت سفارش در پرداخت باید CSS اختصاصی داشته باشد.');

mustContain(styles, 'Hall payment remaining items dropdown.', 'dropdown آیتم‌های پرداخت باید CSS مستقل داشته باشد.');
mustContain(styles, '.hall-payment-popup .hall-remaining-dropdown>summary{list-style:none!important;cursor:pointer!important;display:flex!important;align-items:center!important;justify-content:space-between!important', 'لاین انتخاب همه باید مثل یک summary کشویی یک‌خطی باشد.');
mustContain(styles, '.hall-payment-popup-backdrop{position:fixed!important;inset:0!important;z-index:1200!important;display:flex!important;align-items:flex-start!important;justify-content:center!important;padding:clamp(10px,3vh,24px) clamp(14px,3vw,30px)!important', 'backdrop popup پرداخت باید لایه اول بالاتر از لاین فروش سالن باشد.');
mustContain(styles, 'Hall payment popup third slight lower in-frame app-shell front layer over channel tabs and scroll retention.', 'popup پرداخت باید داخل app-shell و در لایه اول باشد، یک مقدار دیگر پایین‌تر از نسخه قبلی بماند.');
mustContain(styles, '.hall-payment-popup-scroll{max-height:calc(100vh - 158px)!important;overscroll-behavior:contain!important;scrollbar-width:thin!important}', 'scroll داخلی popup باید مستقل و قابل نگهداری باشد و پایین popup قطع نشود.');
mustContain(styles, '.hall-payment-popup{z-index:1201!important;max-height:calc(100vh - 78px)!important;margin-top:calc(-1 * clamp(4px,1vh,8px))!important}', 'خود popup پرداخت باید با z-index بالاتر و margin منفی کمتر، کمی پایین‌تر داخل کادر اپ بماند.');
mustContain(app, "let hallPaymentPopupScrollTop = 0;", 'موقع render مجدد نباید scroll popup پرداخت به اول بپرد.');
mustContain(app, 'data-hall-payment-popup-scroll', 'scroll container popup پرداخت باید قابل شناسایی باشد.');
mustContain(app, "const tabMarkup = renderTab(customer);", 'برای خارج کردن popup پرداخت از content باید tabMarkup جدا ساخته شود.');
mustContain(app, "const floatingHallPaymentOverlay = currentTab === 'sales' ? renderHallPaymentOverlay(customer) : '';", 'popup پرداخت باید جدا از tabMarkup ساخته شود.');
assert(app.indexOf('${floatingHallPaymentOverlay}') < app.indexOf('    <div id="calculatorModalRoot"></div>'), 'overlay پرداخت سالن باید داخل app-shell ولی قبل از خروج از shell/ابزارهای بیرونی قرار بگیرد.');
assert(!app.includes('</div>\n    ${floatingHallPaymentOverlay}\n    <div id="calculatorModalRoot"></div>'), 'popup پرداخت نباید بیرون app-shell برود چون تم و موقعیتش عوض می‌شود.');
assert(!app.includes('${tableOverlays}${renderHallPaymentOverlay(customer)}'), 'popup پرداخت نباید داخل renderHallSales/content باقی بماند.');
mustContain(app, "hallPaymentScroll.scrollTop = hallPaymentPopupScrollTop || 0;", 'بعد از render باید موقعیت scroll قبلی popup برگردد.');
mustContain(app, "hallPaymentScroll.addEventListener('scroll', () => { hallPaymentPopupScrollTop = hallPaymentScroll.scrollTop; }", 'با اسکرول کاربر باید موقعیت scroll popup ذخیره شود.');
mustContain(app, "let hallPaymentItemsDropdownOpen = false;", 'وضعیت باز/بسته بودن کشویی آیتم‌ها باید در render بعدی حفظ شود.');
mustContain(app, "hallPaymentFormLive.querySelector('.hall-remaining-dropdown')?.addEventListener('toggle'", 'کشویی آیتم‌ها باید فقط با کلیک کاربر باز/بسته شود و با refresh خودکار بسته نشود.');
mustContain(app, "hallPaymentFormLive.querySelector('[data-hall-pay-all]')?.addEventListener('click', (event) => { event.stopPropagation(); });", 'کلیک روی خود checkbox انتخاب همه نباید ناخواسته کشویی را باز/بسته کند.');
mustContain(styles, 'Hall payment native small check/radio controls and sticky dropdown state.', 'checkbox خلاصه و radioها باید کنترل native کوچک و بدون خط اضافه باشند.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls>label input[type="radio"],.hall-payment-popup .hall-discount-controls>label input[type="radio"],.hall-payment-popup .hall-remaining-dropdown input[type="checkbox"]{width:16px!important;height:16px!important', 'checkbox خلاصه باید هم‌اندازه checkboxهای آیتم‌ها و radioها کوچک باشد.');
mustContain(styles, 'border:0!important;box-shadow:none!important;background:transparent!important;background-image:none!important;outline:0!important;transform:none!important', 'خط نازک بالای checkbox/radioها باید حذف شود.');
mustContain(styles, '.hall-payment-popup .hall-service-charge-controls>label input[type="radio"]::before,.hall-payment-popup .hall-discount-controls>label input[type="radio"]::before,.hall-payment-popup .hall-remaining-dropdown input[type="checkbox"]::before{content:none!important}', 'pseudo-line بالای radio/checkbox نباید نمایش داده شود.');
assert(!app.includes('این قسمت داخل پرداخت میز است و صندوق‌دار هم می‌تواند درصد یا مبلغ حق سرویس را وارد کند.'), 'جمله توضیحی حق سرویس نباید داخل popup پرداخت نمایش داده شود.');
...
mustContain(restaurantStateApiSource, "for (const key of ['shifts', 'ledger'])", 'endpoint اصلی باید فقط collectionهای افزایشی را merge-by-id کند و restaurantTables را جداگانه با تنظیمات چیدمان replace/filter کند.');
mustContain(restaurantStateApiSource, 'function mergeRestaurantTables(existingTables: any[] = [], incomingTables: any[] = [], customers: any[] = [], orders: any[] = [])', 'endpoint اصلی باید حذف میزهای چیدمان را حفظ کند و میزهای حذف‌شده را از snapshot قدیمی برنگرداند.');
...
mustContain(dashboardSource, 'remove-unused-payment-box-203', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=remove-unused-payment-box-203') && index.includes('core.js?v=remove-unused-payment-box-203') && index.includes('app.js?v=remove-unused-payment-box-203'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
...
assert(app.includes('function showOrderDetailsPreview') && app.includes('orderDetailsModalRoot') && app.includes('data-print-order-details'), 'Cashier order preview decal must open printable details modal.');
assert(app.includes('data-preview-sale') && app.includes('showOrderDetailsPreview(btn.dataset.previewSale)'), 'Cashier order preview decal click handler must be wired.');
assert(app.includes('sale-order-actions') && app.indexOf('deleteButton') < app.indexOf('previewButton') && app.indexOf('previewButton') < app.indexOf('editButton'), 'Cashier popup order actions must render delete, preview, then edit.');
assert(styles.includes('Cashier orders popup preview decal and printable details modal') && styles.includes('.cashier-popup-order-row .order-preview-decal'), 'Cashier order preview decal/modal CSS must be present.');
assert(html.includes('app.js?v=remove-unused-payment-box-203') && html.includes('styles.css?v=remove-unused-payment-box-203'), 'Restaurant static app must cache-bust order preview deploy.');


assert(!app.includes('hall-payment-placeholder') && !app.includes('برای پرداخت یا آزادسازی، روی میز دارای سفارش باز/پرداخت‌شده در لیست میزهای درگیر کلیک کنید.'), 'Unused split bill/payment placeholder box must not render under the cashier.');
assert(app.includes('return `<div class="pos-hall-workspace">${orderForm}</div>${tableOverlays}`;'), 'Hall cashier should render only the order form in pos-hall-workspace unless a real payment popup is opened.');
assert(html.includes('app.js?v=remove-unused-payment-box-203') && html.includes('styles.css?v=remove-unused-payment-box-203'), 'Restaurant static app must cache-bust removal of unused payment placeholder.');
