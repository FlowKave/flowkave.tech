...
mustContain(html, 'styles.css?v=cashier-merge-active-table-locks-153');
mustContain(html, 'core.js?v=cashier-merge-active-table-locks-153');
mustContain(html, 'app.js?v=cashier-merge-active-table-locks-153');
mustContain(html, 'core.js?v=cashier-merge-active-table-locks-153');
...
mustContain(app, 'cashier-merge-active-table-locks-153', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(restaurantStateApiSource, 'existingActive && !incomingActive && hallLockTime(incoming) < timeValue(existing.lockedAt)', 'قفل زرد فعال نباید با release/stale قدیمی دستگاه دیگر پاک شود.');
...
mustContain(publicRestaurantStateApiSource, 'existingActive && !incomingActive && hallLockTime(incoming) < timeValue(existing.lockedAt)', 'قفل زرد فعال در endpoint عمومی هم نباید با state قدیمی پاک شود.');
...
mustContain(app, 'cashier-merge-active-table-locks-153', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(dashboardSource, 'cashier-merge-active-table-locks-153', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-merge-active-table-locks-153') && index.includes('core.js?v=cashier-merge-active-table-locks-153') && index.includes('app.js?v=cashier-merge-active-table-locks-153'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
