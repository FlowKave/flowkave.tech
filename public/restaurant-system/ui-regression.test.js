...
mustContain(html, 'styles.css?v=cashier-payment-popup-lower-192');
mustContain(html, 'core.js?v=cashier-payment-popup-lower-192');
mustContain(html, 'app.js?v=cashier-payment-popup-lower-192');
mustContain(html, 'core.js?v=cashier-payment-popup-lower-192');
...
mustContain(app, 'cashier-payment-popup-lower-192', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-payment-popup-lower-192', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(styles, 'Hall payment popup slightly lower in-frame app-shell front layer over channel tabs and scroll retention.', 'popup پرداخت باید داخل app-shell و در لایه اول باشد، کمی پایین‌تر از نسخه قبلی و بدون خروج از کادر اپ.');
mustContain(styles, '.hall-payment-popup-scroll{max-height:calc(100vh - 152px)!important;overscroll-behavior:contain!important;scrollbar-width:thin!important}', 'scroll داخلی popup باید مستقل و قابل نگهداری باشد و پایین popup قطع نشود.');
mustContain(styles, '.hall-payment-popup{z-index:1201!important;max-height:calc(100vh - 72px)!important;margin-top:calc(-1 * clamp(22px,3vh,34px))!important}', 'خود popup پرداخت باید با z-index بالاتر و margin منفی کمتر، کمی پایین‌تر داخل کادر اپ بماند.');
...
mustContain(dashboardSource, 'cashier-payment-popup-lower-192', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-payment-popup-lower-192') && index.includes('core.js?v=cashier-payment-popup-lower-192') && index.includes('app.js?v=cashier-payment-popup-lower-192'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
