...
mustContain(html, 'styles.css?v=selected-tables-one-row-88');
mustContain(html, 'core.js?v=selected-tables-one-row-88');
mustContain(html, 'app.js?v=selected-tables-one-row-88');
mustContain(html, 'core.js?v=selected-tables-one-row-88');
...
mustContain(styles, '.hall-occupied-tables-scroll{min-height:0!important;max-height:52px!important;overflow-y:auto!important;overflow-x:hidden!important;display:flex!important;flex-wrap:wrap!important', 'میزهای انتخاب‌شده باید فقط یک خط دیده شوند و از خط دوم اسکرول عمودی شود.');
mustContain(styles, 'min-height:48px!important;padding:10px 14px!important', 'دکمه میز انتخاب‌شده باید بزرگ‌تر باشد تا متن بهتر دیده شود.');
...
mustContain(dashboardSource, 'selected-tables-one-row-88', 'Dashboard iframe cache-bust token must match the selected tables one-row layout change.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=selected-tables-one-row-88') && index.includes('core.js?v=selected-tables-one-row-88') && index.includes('app.js?v=selected-tables-one-row-88'), 'cache-bust نمایش یک‌خطی میزهای انتخاب‌شده باید روی نسخه آنلاین هم اعمال شود');
}
