...
assert(app.includes('dashboard-reference-layout') && app.includes('dashboard-left-stack') && app.includes('dashboard-top-cards') && app.includes('dashboard-public-link-panel') && styles.includes('Dashboard reference layout: readiness panel') && styles.includes('grid-template-areas:"left readiness"!important') && styles.includes('.dashboard-left-stack{grid-area:left!important;display:grid!important;grid-template-rows:auto auto!important') && styles.includes('.dashboard-top-cards{display:grid!important;grid-template-columns:minmax(0,1.75fr) minmax(180px,.95fr)!important') && styles.includes('direction:ltr!important}.dashboard-top-cards>.panel{direction:rtl!important'), 'Dashboard reference layout must match screenshot: readiness right, stock/status top-left, public link under them.');
...
mustContain(html, 'styles.css?v=dashboard-layout-reference-19');
mustContain(html, 'core.js?v=dashboard-layout-reference-19');
mustContain(html, 'app.js?v=dashboard-layout-reference-19');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=dashboard-layout-reference-19') && index.includes('core.js?v=dashboard-layout-reference-19') && index.includes('app.js?v=dashboard-layout-reference-19'), 'cache-bust هماهنگی تم باید روی نسخه آنلاین هم اعمال شود');
}
