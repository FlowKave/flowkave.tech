...
mustContain(html, 'styles.css?v=cashier-close-ignores-draft-locks-156');
mustContain(html, 'core.js?v=cashier-close-ignores-draft-locks-156');
mustContain(html, 'app.js?v=cashier-close-ignores-draft-locks-156');
mustContain(html, 'core.js?v=cashier-close-ignores-draft-locks-156');
...
mustContain(app, 'cashier-close-ignores-draft-locks-156', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-close-ignores-draft-locks-156', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(app, "const visualStatus = isDraftLocked ? 'draft-locked' : table.status;", 'بعد از ثبت سفارش، status میز باید open-order بماند و قرمز شود حتی اگر lock stale مانده باشد.');
...
mustContain(dashboardSource, 'cashier-close-ignores-draft-locks-156', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-close-ignores-draft-locks-156') && index.includes('core.js?v=cashier-close-ignores-draft-locks-156') && index.includes('app.js?v=cashier-close-ignores-draft-locks-156'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
