...
mustContain(html, 'styles.css?v=qr-receipt-empty-inputs-113');
mustContain(html, 'core.js?v=qr-receipt-empty-inputs-113');
mustContain(html, 'app.js?v=qr-receipt-empty-inputs-113');
mustContain(html, 'core.js?v=qr-receipt-empty-inputs-113');
...
mustContain(app, "if (publicQrTableBlocked(table)) return alert('این میز الان در صندوق سفارش باز دارد", 'اگر میز در صندوق درگیر باشد QR نباید اجازه ثبت سفارش بدهد.');
...
mustContain(dashboardSource, 'qr-receipt-empty-inputs-113', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=qr-receipt-empty-inputs-113') && index.includes('core.js?v=qr-receipt-empty-inputs-113') && index.includes('app.js?v=qr-receipt-empty-inputs-113'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
