...
mustContain(html, 'styles.css?v=cashier-payment-service-spacing-177');
mustContain(html, 'core.js?v=cashier-payment-service-spacing-177');
mustContain(html, 'app.js?v=cashier-payment-service-spacing-177');
mustContain(html, 'core.js?v=cashier-payment-service-spacing-177');
...
mustContain(app, 'cashier-payment-service-spacing-177', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-payment-service-spacing-177', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(app, "const hasServiceChoice = Object.prototype.hasOwnProperty.call(order, 'serviceChargeMode');", 'حق سرویس پیش‌فرض فقط وقتی هنوز انتخابی برای فیش ثبت نشده باشد اعمال شود.');
mustContain(app, "const mode = hasServiceChoice ? (order.serviceChargeMode || '') : 'percent';", 'حق سرویس باید به صورت پیش‌فرض روی محاسبه درصدی باشد.');
mustContain(app, "Number(hasServiceChoice ? (order.serviceChargePercent || 0) : 5)", 'درصد پیش‌فرض حق سرویس باید ۵٪ باشد ولی بعد از حذف کاربر دوباره خودکار برنگردد.');
...
mustContain(dashboardSource, 'cashier-payment-service-spacing-177', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-payment-service-spacing-177') && index.includes('core.js?v=cashier-payment-service-spacing-177') && index.includes('app.js?v=cashier-payment-service-spacing-177'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
