...
mustContain(html, 'styles.css?v=cashier-payment-popup-header-174');
mustContain(html, 'core.js?v=cashier-payment-popup-header-174');
mustContain(html, 'app.js?v=cashier-payment-popup-header-174');
mustContain(html, 'core.js?v=cashier-payment-popup-header-174');
...
mustContain(app, 'cashier-payment-popup-header-174', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-payment-popup-header-174', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(app, 'شماره سفارش: <b>${receiptNumberText(order.trackingNumber || 0)}</b>', 'header popup باید شماره سفارش را نشان بدهد.');
mustContain(app, 'مالیات: <b>${money(order.taxTotal || 0)}</b>', 'header popup باید مالیات را نشان بدهد.');
mustContain(app, 'مبلغ کل: <b data-hall-summary-grand>${money(orderFinalTotal(order))}</b>', 'header popup باید مبلغ کل را با data update نشان بدهد.');
mustContain(app, 'type="submit" form="hallPaymentForm"', 'دکمه ثبت پرداخت header باید همان فرم پرداخت قبلی را submit کند.');
mustContain(app, 'data-close-hall-payment-popup>افزودن به سفارش</button>', 'دکمه افزودن به سفارش باید popup پرداخت را ببندد و کاربر را به سفارش همان میز برگرداند.');
...
mustContain(dashboardSource, 'cashier-payment-popup-header-174', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-payment-popup-header-174') && index.includes('core.js?v=cashier-payment-popup-header-174') && index.includes('app.js?v=cashier-payment-popup-header-174'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
