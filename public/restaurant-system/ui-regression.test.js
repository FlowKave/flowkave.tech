...
mustContain(html, 'styles.css?v=cashier-qr-no-relock-after-submit-170');
mustContain(html, 'core.js?v=cashier-qr-no-relock-after-submit-170');
mustContain(html, 'app.js?v=cashier-qr-no-relock-after-submit-170');
mustContain(html, 'core.js?v=cashier-qr-no-relock-after-submit-170');
...
mustContain(app, 'cashier-qr-no-relock-after-submit-170', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-qr-no-relock-after-submit-170', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(publicRestaurantStateApiSource, "for (const key of ['shifts', 'ledger', 'restaurantTables'])", 'endpoint عمومی باید روز کاری صندوق، ledger و میزها را با state تبلت merge کند.');
...
mustContain(restaurantStateApiSource, "for (const key of ['shifts', 'ledger', 'restaurantTables'])", 'endpoint اصلی باید روز کاری، ledger و میزها را merge کند.');
...
mustContain(dashboardSource, 'cashier-qr-no-relock-after-submit-170', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-qr-no-relock-after-submit-170') && index.includes('core.js?v=cashier-qr-no-relock-after-submit-170') && index.includes('app.js?v=cashier-qr-no-relock-after-submit-170'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
