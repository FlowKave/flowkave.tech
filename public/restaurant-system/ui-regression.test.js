...
mustContain(html, 'styles.css?v=customer-settings-cleanup-22');
mustContain(html, 'core.js?v=customer-settings-cleanup-22');
mustContain(html, 'app.js?v=customer-settings-cleanup-22');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=customer-settings-cleanup-22') && index.includes('core.js?v=customer-settings-cleanup-22') && index.includes('app.js?v=customer-settings-cleanup-22'), 'cache-bust هماهنگی تم باید روی نسخه آنلاین هم اعمال شود');
}
...
assert(app.includes("['account','تنظیمات']") && !app.includes("['account','مشتری/پکیج']"), 'Customer/package nav label must become settings.');
...
assert(accountSource.includes('تعریف کارکنان مجاز') && accountSource.includes('دعوت کارکنان') && accountSource.includes('بازیابی رمز عبور') && accountSource.includes('پشتیبان‌گیری و بازیابی') && accountSource.includes('مشخصات رستوران'), 'Useful operational settings must remain visible.');
