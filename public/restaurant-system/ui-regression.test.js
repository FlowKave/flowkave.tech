...
mustContain(html, 'styles.css?v=cashier-payment-note-box-195');
mustContain(html, 'core.js?v=cashier-payment-note-box-195');
mustContain(html, 'app.js?v=cashier-payment-note-box-195');
mustContain(html, 'core.js?v=cashier-payment-note-box-195');
...
mustContain(app, 'cashier-payment-note-box-195', 'لینک‌های QR/رسید باید token جدید باز/بستن صندوق shared را داشته باشند.');
...
mustContain(app, 'cashier-payment-note-box-195', 'لینک‌های QR و صندوق باید token جدید نسخه جاری را داشته باشند.');
...
mustContain(styles, 'Hall payment popup second slight lower in-frame app-shell front layer over channel tabs and scroll retention.', 'popup پرداخت باید داخل app-shell و در لایه اول باشد، یک مقدار دیگر پایین‌تر از نسخه قبلی بماند.');
...
mustContain(styles, '.hall-payment-popup{z-index:1201!important;max-height:calc(100vh - 78px)!important;margin-top:calc(-1 * clamp(10px,2vh,18px))!important}', 'خود popup پرداخت باید با z-index بالاتر و margin منفی خیلی کم، کمی پایین‌تر داخل کادر اپ بماند.');
...
mustContain(dashboardSource, 'cashier-payment-note-box-195', 'Dashboard iframe cache-bust token must match the VAT open-order fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=cashier-payment-note-box-195') && index.includes('core.js?v=cashier-payment-note-box-195') && index.includes('app.js?v=cashier-payment-note-box-195'), 'cache-bust اصلاح اعمال مالیات روی فیش باز باید روی نسخه آنلاین هم اعمال شود');
}
