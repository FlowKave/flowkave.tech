...
mustContain(html, 'styles.css?v=order-status-simplified-97');
mustContain(html, 'core.js?v=order-status-simplified-97');
mustContain(html, 'app.js?v=order-status-simplified-97');
mustContain(html, 'core.js?v=order-status-simplified-97');
...
assert(!salesSource.slice(salesSource.indexOf('const statusPanel'), salesSource.indexOf('if (posSalesChannel === \'hall\')')).includes('میز:'), 'نمایش میز باید از وضعیت سفارشات حذف شود.');
...
mustContain(styles, '.order-status-row>b{justify-self:center;text-align:center', 'مبلغ سمت راست وضعیت سفارشات باید از لبه راست فاصله بگیرد و وسط‌تر باشد.');
...
mustContain(dashboardSource, 'order-status-simplified-97', 'Dashboard iframe cache-bust token must match the order status simplification fix.');
...
function testThemeHarmonyForCashierTablesAndPos() {
  ...
  assert(index.includes('styles.css?v=order-status-simplified-97') && index.includes('core.js?v=order-status-simplified-97') && index.includes('app.js?v=order-status-simplified-97'), 'cache-bust اصلاح وضعیت سفارشات باید روی نسخه آنلاین هم اعمال شود');
}
