...
function testOpenHallOrderCanBeEditedAndPaidDeleteRefunds() {
  const state = RestaurantCore.createInitialState();
  const customer = RestaurantCore.createCustomer(state, { businessName:'خان بابا', ownerName:'مالک', phone:'09120000004', email:'popup-orders@test.local', password:'123456' });
  const menu = RestaurantCore.createMenu(state, customer.id, { name:'منو' });
  const tea = RestaurantCore.createMenuItem(state, customer.id, menu.id, { name:'چای', price:10000 });
  const cake = RestaurantCore.createMenuItem(state, customer.id, menu.id, { name:'کیک', price:50000 });
  ...
  assert.equal(deleted.id, edited.id);
