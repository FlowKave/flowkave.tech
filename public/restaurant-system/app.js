...
function render() {
  ...
  const navItems = [
    ['dashboard','داشبورد'],['customerBank','باشگاه مشتریان'],['aiAssistant','هوش مصنوعی'],['menu','منو'],['sales','صندوق'],['recipes','رسپی'],['inventory','انبار'],['accounting','حسابداری'],['account','تنظیمات']
...
function titleFor(tab){return {dashboard:'داشبورد عملیاتی',customerBank:'باشگاه مشتریان و بازگشت مشتری',aiAssistant:'هوش مصنوعی عملیاتی',menu:'ساخت و مدیریت منوی دیجیتال',sales:'صندوق و ثبت سفارش',recipes:'رسپی و قیمت تمام‌شده',inventory:'انبارگردانی',accounting:'حسابداری پایه',account:'تنظیمات رستوران'}[tab] || 'داشبورد'}
...
function renderAccount(customer) {
  ...
  return `<section class="workspace settings-workspace">
    <form class="panel" id="staffForm"><h2>تعریف کارکنان مجاز</h2><p>در این بخش کارکنان مجاز رستوران را تعریف کنید؛ حساب مالک در این لیست نمایش داده نمی‌شود و ورود کارکنان با کد پرسنلی و پین است.</p><label>نام کارمند<input name="name" value="صندوق‌دار شیفت"></label><label>کد پرسنلی<input name="personnelCode" value="۱۰۰۱" inputmode="numeric" dir="ltr" autocomplete="off"></label><label>پین کد<input name="pin" value="۱۲۳۴" type="password" inputmode="numeric"></label><label>نقش<select name="role"><option value="cashier">صندوق‌دار</option><option value="manager">مدیر</option></select></label><button class="primary">افزودن کارمند</button></form>
    <form class="panel" id="invitationForm"><h2>دعوت کارکنان</h2><p>دعوت کارکنان برای ساخت دسترسی کنترل‌شده است و تا زمان پذیرش، کاربر فعال عملیاتی نمی‌سازد.</p><label>نام دعوت‌شونده<input name="name" value="صندوق‌دار تازه"></label><label>ایمیل دعوت<input name="email" value="invite${Date.now().toString().slice(-4)}@restaurant.test" type="email" dir="ltr" autocomplete="email"></label><label>نقش<select name="role"><option value="cashier">صندوق‌دار</option><option value="manager">مدیر</option></select></label><button class="primary">ساخت دعوت کارکنان</button></form>
    ...
    <div class="panel wide staff-users-panel"><h2>کارکنان تعریف‌شده</h2><p>این لیست فقط کارکنانی را نشان می‌دهد که مالک تعریف کرده است؛ حساب مالک اینجا ثبت نمی‌شود. مدیر می‌تواند نام، کد پرسنلی، پین جدید، نقش، وضعیت و حذف هر کارمند را مدیریت کند.</p><div class="staff-user-list">${staffUsers.map(renderStaffUserManagementRow).join('') || '<p>هنوز کارمندی تعریف نشده است.</p>'}</div></div>
