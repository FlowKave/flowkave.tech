...
let hallPaymentPopupOrderId = '';
let hallPaymentItemsDropdownOpen = false;
let hallPaymentPopupScrollTop = 0;
...
function showHallPaymentTableReleaseChoice(form) {
  ...
  modal.innerHTML = `<section class="receipt-choice-modal hall-payment-release-choice-modal" role="dialog" aria-modal="true" aria-label="انتخاب وضعیت میز بعد از پرداخت">
    ...
    <p>وضعیت میز را انتخاب کنید.</p>
    <div class="receipt-choice-actions hall-payment-release-choice-actions"><button type="button" class="primary" data-pay-and-free-table>میز آزاد</button><button type="button" class="secondary" data-pay-and-hold-table>میز درگیر</button></div>
  </section>`;
  document.body.appendChild(modal);
  const submitWithChoice = (choice) => {
    form.dataset.freeTableAfterPaymentChoice = choice;
    modal.remove();
    form.requestSubmit();
  };
  modal.addEventListener('click', (event) => { if (event.target === modal) modal.remove(); });
...
function publicQrOwnActiveTableLock(table) {
  const lock = table ? activeHallTableLock(table.customerId, table.id) : null;
  return Boolean(lock && lock.ownerId === hallTableLockOwnerId());
}
...
async function ensurePublicQrTableLock(customerId, table) {
  ...
  try {
    ...
    console.warn('public QR table lock failed', error);
  }
}
...
function render() {
  ...
  const tabMarkup = renderTab(customer);
  const floatingHallPaymentOverlay = currentTab === 'sales' ? renderHallPaymentOverlay(customer) : '';
  app.innerHTML = `
...
function renderPublicMenu(customerId) {
  ...
  const qrOrderingLocked = Boolean(table && tableIsBlocked);
...
function renderHallPaymentOverlay(customer) {
  if (!hallPaymentPopupOrderId) return '';
  const order = (state.orders || []).find(item => item.id === hallPaymentPopupOrderId && item.customerId === customer.id);
  const stillPayable = order && order.hallSale === true && order.posStatus !== 'paid';
  if (!stillPayable) { hallPaymentPopupOrderId = ''; return ''; }
  const openOrderHeader = `<div class="hall-payment-popup-title"><button type="button" class="modal-close-icon hall-payment-popup-close" data-close-hall-payment-popup aria-label="بستن">×</button><div class="hall-payment-popup-meta"><span class="hall-payment-popup-stat"><small>شماره سفارش</small><b>${receiptNumberText(order.trackingNumber || 0)}</b></span><span class="hall-payment-popup-stat"><small>مالیات</small><b>${money(order.taxTotal || 0)}</b></span><span class="hall-payment-popup-stat"><small>مبلغ کل</small><b data-hall-summary-grand>${money(orderFinalTotal(order))}</b></span><button class="primary" type="submit" form="hallPaymentForm" ${RestaurantCore.getRemainingPaymentItems(order).length ? '' : 'disabled'}>ثبت پرداخت</button><button class="secondary" type="button" data-close-hall-payment-popup>افزودن آیتم</button></div></div>`;
  return `<div class="modal-backdrop hall-payment-popup-backdrop" data-close-hall-payment-popup><section class="hall-payment-popup" role="dialog" aria-modal="true" aria-label="تقسیم فیش و پرداخت میز">${openOrderHeader}<div class="hall-payment-popup-scroll" data-hall-payment-popup-scroll>${renderHallPaymentPanel(order)}</div></section></div>`;
}

function renderHallServiceChargeControls(order) {
  const serviceCleared = order.serviceChargeDefaultCleared === true;
  const hasSavedService = Boolean(order.serviceChargeMode) || Number(order.serviceChargePercent || 0) > 0 || Number(order.serviceChargeAmount || 0) > 0 || Number(order.serviceChargeTotal || 0) > 0;
...
function renderHallDiscountControls(order) {
  const mode = order.discountMode || '';
  const percentValue = mode === 'percent' ? Number(order.discountPercent || 0) : 0;
  const amountValue = mode === 'percent' ? Math.round(Number(order.subtotal ?? order.total ?? 0) * percentValue / 100) : (mode === 'amount' ? Number(order.discountAmount || order.discountTotal || 0) : 0);
  return `<div class="hall-service-charge-box hall-discount-box" data-hall-discount-box><div class="hall-service-charge-controls hall-discount-controls"><label><span><strong>تخفیف</strong> — محاسبه درصدی</span><input type="radio" name="discountMode" value="percent" ${mode === 'percent' ? 'checked' : ''}><input name="discountPercent" data-number inputmode="decimal" value="${percentValue ? numberText(percentValue,2) : ''}" aria-label="درصد تخفیف"><b>٪</b></label><label><span>مبلغ</span><input type="radio" name="discountMode" value="amount" ${mode === 'amount' ? 'checked' : ''}><input name="discountAmount" data-number data-money inputmode="decimal" value="${amountValue ? numberText(amountValue,0) : ''}" ${mode === 'percent' ? 'readonly' : ''} aria-label="مبلغ تخفیف"><b>تومان</b></label><button type="button" class="secondary" data-clear-discount>حذف تخفیف</button></div></div>`;
}
...
function bindCommon() {
  ...
  document.querySelectorAll('[data-hall-occupied-table]').forEach(btn => btn.addEventListener('click', () => {
    ...
    const table = RestaurantCore.getHallTables(state, customer.id).find(item => item.id === tableId);
    const activeOrder = table ? RestaurantCore.getActiveHallOrder(state, customer.id, table.id) : null;
    selectedHallTableId = tableId;
    hallPaymentPopupOrderId = activeOrder?.id || '';
    hallPaymentItemsDropdownOpen = false;
    hallPaymentPopupScrollTop = 0;
    hallTablePickerOpen = false;
    render();
  }));
  document.querySelectorAll('[data-close-hall-payment-popup]').forEach(btn => btn.addEventListener('click', (event) => {
    if (event.target !== btn && event.target.closest('.hall-payment-popup')) return;
    hallPaymentPopupOrderId = '';
    hallPaymentItemsDropdownOpen = false;
    hallPaymentPopupScrollTop = 0;
    render();
  }));
  ...
  document.querySelectorAll('[data-release-hall-table-lock]').forEach(btn => btn.addEventListener('click', async () => {
  ...
  const applyHallServiceChargeFromForm = () => {
    ...
    let updatedOrder = RestaurantCore.setOrderServiceCharge(state, customer.id, hallPaymentFormLive.dataset.orderId, {
    ...
    if (RestaurantCore.setOrderDiscount) updatedOrder = RestaurantCore.setOrderDiscount(state, customer.id, hallPaymentFormLive.dataset.orderId, {
      discountMode: f.get('discountMode') || '',
      discountPercent: parseFaNumber(f.get('discountPercent') || 0),
      discountAmount: parseFaNumber(f.get('discountAmount') || 0),
    });
    return updatedOrder;
  };
  const refreshHallPaymentPreview = () => {
    ...
    const itemsSubtotalEl = hallPaymentFormLive.querySelector('[data-hall-items-subtotal]');
    if (itemsSubtotalEl) {
      const selectedSubtotal = selected.reduce((sum, item) => {
        const row = hallPaymentFormLive.querySelector(`[data-hall-pay-line][value="${CSS.escape(item.lineId)}"]`)?.closest('.hall-pay-item');
        const unitText = row?.querySelector('[data-hall-unit-price]')?.dataset.hallUnitPrice || 0;
        return sum + (Number(unitText || 0) * Number(item.qty || 0));
      }, 0);
  ...
  if (hallPaymentFormLive) {
    ...
    if (hallPaymentScroll) {
      hallPaymentScroll.scrollTop = hallPaymentPopupScrollTop || 0;
      hallPaymentScroll.addEventListener('scroll', () => { hallPaymentPopupScrollTop = hallPaymentScroll.scrollTop; }, { passive: true });
    }
    ...
    hallPaymentFormLive.addEventListener('change', (event) => {
      ...
      if (event.target.matches('[data-clear-discount]')) {
        hallPaymentFormLive.querySelectorAll('[name="discountMode"]').forEach(input => { input.checked = false; });
        const percent = hallPaymentFormLive.querySelector('[name="discountPercent"]');
        const amount = hallPaymentFormLive.querySelector('[name="discountAmount"]');
        if (percent) percent.value = '';
        if (amount) amount.value = '';
      }
    ...
    hallPaymentFormLive.querySelector('[data-clear-service-charge]')?.addEventListener('click', () => {
      const order = (state.orders || []).find(item => item.id === hallPaymentFormLive.dataset.orderId && item.customerId === customer.id);
      if (order) order.serviceChargeDefaultCleared = true;
      hallPaymentFormLive.querySelectorAll('[name="serviceMode"]').forEach(input => { input.checked = false; });
      const percent = hallPaymentFormLive.querySelector('[name="servicePercent"]');
      const amount = hallPaymentFormLive.querySelector('[name="serviceAmount"]');
    ...
    hallPaymentFormLive.querySelector('[data-clear-discount]')?.addEventListener('click', () => {
      hallPaymentFormLive.querySelectorAll('[name="discountMode"]').forEach(input => { input.checked = false; });
      const percent = hallPaymentFormLive.querySelector('[name="discountPercent"]');
      const amount = hallPaymentFormLive.querySelector('[name="discountAmount"]');
      if (percent) percent.value = numberText(0, 2);
  ...
  document.querySelectorAll('[data-delete-sale]').forEach(btn => btn.addEventListener('click', async () => {
    ...
    try {
      const order = (state.orders || []).find(item => item.id === btn.dataset.deleteSale && item.customerId === customer.id);
      RestaurantCore.deleteSale(state, customer.id, btn.dataset.deleteSale);
      if (order?.tableId && selectedHallTableId === order.tableId) selectedHallTableId = '';
      await persistCriticalState();
      render();
    }
