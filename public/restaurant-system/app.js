...
function formatNationalIdInput(value) {
  ...
  if (digits.length <= 9) return `${faNum(digits.slice(3))}-${faNum(digits.slice(0, 3))}`;
  return `${faNum(digits.slice(9, 10))}-${faNum(digits.slice(3, 9))}-${faNum(digits.slice(0, 3))}`;
}
