...
function isMoneyFieldName(name = '') { return /amount|price|cost|balance|unitCost|openingBalance/i.test(String(name)); }
...
function bindPersianNumberInputs(scope = document) { scope.querySelectorAll('[data-number]').forEach(input => input.addEventListener('input', () => { input.value = input.hasAttribute('data-money') ? formatGroupedNumberInput(input.value) : faNum(toEnglishDigits(input.value).replace(/[^0-9.٬,٫]/g, '')); })); }
...
function showHallOrderReceiptPrintPreview(order, options = {}) {
  ...
  modal.innerHTML = `<section class="print-modal hall-receipt-modal" role="dialog" aria-modal="true" aria-label="فیش سفارش سالن">
    ...
    <div class="recipe-print-sheet hall-receipt-sheet">
      <header class="recipe-print-header"><h1>فیش سفارش ${esc(order.tableName || '')}</h1><strong>شماره پیگیری ${numberText(order.trackingNumber || 0,0)}</strong></header>
      ...
      <section class="recipe-print-box hall-receipt-total"><h2>جمع کل</h2><strong>${money(order.grandTotal || order.total || 0)}</strong><small>${formatDate(order.createdAt)}</small></section>
    </div>
  </section>`;
  document.body.appendChild(modal);
...
function showKitchenTicketPrintPreview(orderId) {
  ...
  modal.innerHTML = `<section class="print-modal" role="dialog" aria-modal="true">
    ...
    <div class="recipe-print-sheet station-ticket-sheet">
      <header class="recipe-print-header"><h1>شماره پیگیری ${numberText(order.trackingNumber || 0,0)}</h1><strong>${esc(orderStatusLabel(order.status))}</strong></header>
...
function showKitchenStationQueuePrintPreview(station, filter = kitchenQueueFilter) {
  ...
  const ticketSections = tickets.map(ticket => `<section class="recipe-print-box station-queue-ticket ${ticket.delayed ? 'station-queue-ticket-delayed' : ''}"><h2>شماره پیگیری ${numberText(ticket.trackingNumber || 0,0)} — ${esc(orderStatusLabel(ticket.status))}</h2>${orderGuestLine(ticket) ? `<p>${orderGuestLine(ticket)}</p>` : ''}${ticket.orderNote ? `<p>یادداشت سفارش: ${esc(ticket.orderNote)}</p>` : ''}<div class="station-ticket-lines">${ticket.lines.map((line, index) => `<div class="station-ticket-line"><span class="recipe-print-number">${numberText(index + 1,0)}</span><b>${esc(line.name)}</b><strong>${numberText(line.qty,0)}</strong>${line.note || line.modifiers?.length ? `<small>${esc([line.note, ...(line.modifiers || [])].filter(Boolean).join('، '))}</small>` : ''}</div>`).join('')}</div><small>زمان دریافت: ${formatDate(ticket.createdAt)}</small></section>`).join('');
  modal.innerHTML = `<section class="print-modal" role="dialog" aria-modal="true">
...
function renderDashboard(customer) {
  ...
  return `<section class="workspace"><div class="panel wide"><div class="section-title"><h2>مسیر تست واقعی</h2><span class="badge">قابل کلیک</span></div><div class="flow"><div>۱. مشتری/اکانت ساخته شده</div><div>۲. منوی دیجیتال بساز</div><div>۳. مواد اولیه و رسپی وارد کن</div><div>۴. فروش ثبت کن</div><div>۵. انبار و حسابداری تغییر می‌کند</div></div></div>${renderDashboardReadinessShortcuts(customer)}<div class="panel"><h2>وضعیت</h2><p>آیتم منو: <b>${numberText(items.length,0)}</b></p><p>فروش ثبت‌شده: <b>${numberText(orders.length,0)}</b></p><p>مواد اولیه: <b>${numberText(inv.length,0)}</b></p></div><div class="panel ${lowStock.length ? 'danger' : ''}"><h2>هشدار کمبود موجودی</h2>${lowStock.length ? `<p>${esc(lowStockText(lowStock))}</p>` : '<p>موجودی بحرانی ندارید.</p>'}</div><div class="panel"><h2>لینک منوی عمومی / کد پاسخ سریع</h2><p>این لینک صفحه مشتری را باز می‌کند؛ سفارش ثبت‌شده از همین مسیر وارد فروش و انبار می‌شود.</p><a class="public-link" href="#menu/${encodeURIComponent(customer.id)}">باز کردن منوی عمومی</a><code>${esc(publicLink)}</code></div></section>`;
}
...
function bindPublicMenu(customerId, items) {
  ...
  form.addEventListener('submit', (e) => {
    ...
    message.innerHTML = `<b>سفارش شما با شماره پیگیری ${numberText(order.trackingNumber, 0)} ثبت شد.</b><span>مبلغ: ${money(order.total)} — وضعیت: ${orderStatusLabel(order.status)}</span>${order.lowStockWarnings.length ? '<small>هشدار کمبود برای اپراتور ثبت شد.</small>' : ''}`;
...
function renderPublicTrackingResult(order) {
  ...
  return `<b>شماره پیگیری ${numberText(order.trackingNumber, 0)}</b><span>وضعیت: ${orderStatusLabel(order.status)}</span><span>مبلغ: ${money(order.total)}</span>${guest}<small>آیتم‌ها: ${order.lines.map(line => `${esc(line.name)} × ${numberText(line.qty, 0)}`).join('، ')}</small>${renderOrderPrepNotes(order)}`;
}
...
function renderSales(customer) {
  if (posSalesChannel !== 'hall') return `<section class="workspace pos-workspace"><div class="panel wide">${posChannelTabs()}<h2>${posSalesChannel === 'delivery' ? 'دلیوری' : 'اسنپ‌فود'}</h2><p>در این مرحله فقط ورودی مستقل این بخش آماده شده و منطق آن در فاز بعد پیاده‌سازی می‌شود.</p></div></section>`;
  ...
  const statusPanel = `<div class="panel wide order-status-panel"><h2>فروش‌ها و وضعیت سفارش</h2><p>سفارش‌ها و پرداخت‌های سالن؛ پرداخت هر میز می‌تواند کامل یا آیتمی/جزئی ثبت شود.</p>${orders.map(o=>`<div class="order-row order-status-row ${o.lowStockWarnings?.length ? 'danger' : ''}"><b>${money(o.remainingTotal ?? o.total)}</b><span><strong>شماره پیگیری ${numberText(o.trackingNumber || 0,0)} — ${esc(orderStatusLabel(o.status))}</strong>${o.tableName ? `<br><small>میز: ${esc(o.tableName)} — وضعیت صندوق: ${esc(o.posStatus === 'paid' ? 'تسویه‌شده' : o.posStatus === 'partially-paid' ? 'پرداخت جزئی' : 'سفارش باز')}</small>` : ''}${orderGuestLine(o) ? `<br><small>${orderGuestLine(o)}</small>` : ''}<br>${o.lines.map(l=>`${esc(l.name)} × ${numberText(l.qty,0)}`).join('، ')}${renderOrderPrepNotes(o)}${o.payments?.length ? `<br><small>پرداخت‌ها: ${o.payments.map(p => `${money(p.amount)} ${esc(p.paymentMethod || '')}`).join('، ')}</small>` : ''}${o.completedAt ? `<br><small>زمان تحویل: ${formatDate(o.completedAt)}</small>` : ''}${o.lowStockWarnings?.length ? `<br><small>هشدار کمبود: ${esc(lowStockText(o.lowStockWarnings))}</small>` : ''}</span><em>${formatDate(o.createdAt)}</em><label>وضعیت<select data-order-status="${o.id}">${orderStatusOptions(o.status || 'completed')}</select></label><div class="row-action-buttons">${actionDecalButton('edit', `data-edit-sale="${o.id}"`, 'sale-row-decal', 'ویرایش فروش')}${actionDecalButton('delete', `data-delete-sale="${o.id}"`, 'sale-row-decal', 'حذف فروش')}</div></div>`).join('') || '<p>هنوز فروشی ثبت نشده.</p>'}</div>`;
  if (posSalesChannel === 'hall') return `<section class="workspace pos-workspace"><div class="panel wide">${posChannelTabs()}</div><div class="panel wide pos-hall-shell">${hall}</div>${renderKitchenOrderQueue(customer)}<div class="panel wide order-completion-summary"><h2>خلاصه تحویل سفارش</h2><p>${esc(orderCompletionSummaryText(completionSummary))}</p></div>${statusPanel}</section>`;
  ...
  return `<section class="workspace pos-workspace"><div class="panel wide">${posChannelTabs()}</div><div class="panel wide pos-hall-shell">${hall}</div><form class="panel" id="saleForm"><h2>${editingOrder ? 'ویرایش فروش' : 'ثبت فروش سبدی'}</h2><p>برای یک فاکتور چند آیتم انتخاب کن؛ ردیف‌های با تعداد صفر نادیده گرفته می‌شوند.</p><input type="hidden" name="editingOrderId" value="${esc(editingOrder?.id || '')}"><div id="saleRows">${starterRows}</div><button type="button" class="secondary" id="addSaleLine">+ افزودن آیتم به فاکتور</button><label>یادداشت سفارش برای آشپزخانه<textarea name="orderNote" rows="۳" placeholder="مثلا عجله‌ای، بسته‌بندی جدا، بدون کارد و چنگال">${esc(editingOrder?.orderNote || '')}</textarea></label><label>روش پرداخت<select name="payment"><option value="card" ${paymentSelected('card')}>کارتخوان</option><option value="cash" ${paymentSelected('cash')}>نقدی</option><option value="online" ${paymentSelected('online')}>آنلاین</option></select></label><button class="primary">${editingOrder ? 'ذخیره ویرایش فروش' : 'ثبت فاکتور و کاهش انبار'}</button>${editingOrder ? '<button type="button" class="secondary" data-cancel-sale-edit>انصراف از ویرایش</button>' : ''}</form>${renderKitchenOrderQueue(customer)}<div class="panel wide order-completion-summary"><h2>خلاصه تحویل سفارش</h2><p>${esc(orderCompletionSummaryText(completionSummary))}</p></div><div class="panel wide order-status-panel"><h2>فروش‌ها و وضعیت سفارش</h2><p>فروش‌های ثبت‌شده را می‌توان ویرایش یا حذف کرد؛ حذف فروش اثر موجودی و دفتر مالی همان فاکتور را برمی‌گرداند.</p>${orders.map(o=>`<div class="order-row order-status-row ${o.lowStockWarnings?.length ? 'danger' : ''}"><b>${money(o.total)}</b><span><strong>شماره پیگیری ${numberText(o.trackingNumber || 0,0)} — ${esc(orderStatusLabel(o.status))}</strong>${orderGuestLine(o) ? `<br><small>${orderGuestLine(o)}</small>` : ''}<br>${o.lines.map(l=>`${esc(l.name)} × ${numberText(l.qty,0)}`).join('، ')}${renderOrderPrepNotes(o)}${o.completedAt ? `<br><small>زمان تحویل: ${formatDate(o.completedAt)}</small>` : ''}${o.lowStockWarnings?.length ? `<br><small>هشدار کمبود: ${esc(lowStockText(o.lowStockWarnings))}</small>` : ''}</span><em>${formatDate(o.createdAt)}</em><label>وضعیت<select data-order-status="${o.id}">${orderStatusOptions(o.status || 'completed')}</select></label><div class="row-action-buttons">${actionDecalButton('edit', `data-edit-sale="${o.id}"`, 'sale-row-decal', 'ویرایش فروش')}${actionDecalButton('delete', `data-delete-sale="${o.id}"`, 'sale-row-decal', 'حذف فروش')}</div></div>`).join('') || '<p>هنوز فروشی ثبت نشده.</p>'}</div></section>`;
}
...
function renderStaffUserManagementRow(user) {
  ...
  return `<form class="staff-user-edit-form order-row" data-staff-user-id="${esc(user.id)}"><div class="staff-user-edit-grid"><label>نام کارمند<input name="name" value="${esc(user.name || '')}"></label><label>کد پرسنلی<input name="personnelCode" value="${esc(faNum(user.personnelCode || ''))}" inputmode="numeric" dir="ltr" autocomplete="off"></label><label>پین کد جدید<input name="pin" value="" type="password" inputmode="numeric" placeholder="در صورت نیاز وارد کن"></label><label>نقش<select name="role"><option value="cashier" ${user.role === 'cashier' ? 'selected' : ''}>صندوق‌دار</option><option value="manager" ${user.role !== 'cashier' ? 'selected' : ''}>مدیر</option></select></label></div><span class="staff-user-status">وضعیت: ${isInactive ? 'غیرفعال' : 'فعال'} — ورود با کد پرسنلی و پین</span><div class="row-action-buttons staff-user-actions"><button type="submit" class="secondary">ذخیره تغییرات</button><button type="button" class="secondary" data-toggle-staff="${esc(user.id)}">${isInactive ? 'فعال‌سازی' : 'غیرفعال‌سازی'}</button>${actionDecalButton('delete', `data-delete-staff="${esc(user.id)}"`, 'staff-user-decal', 'حذف کارمند')}</div></form>`;
}

function renderAccount(customer) {
  const migrationPlan = RestaurantCore.getMvpMigrationPlan();
  const authPlan = migrationPlan.authMigration || { sessionTableFields: [], requestProtection: [], routeGuards: [], passwordRecovery: [], staffInvitation: [] };
  ...
  const securityEvents = RestaurantCore.getSecurityEvents ? RestaurantCore.getSecurityEvents(state, customer.id, securityEventFilterPayload()) : [];
  const selected = (value, current) => value === current ? 'selected' : '';
  ...
  return `<section class="workspace">
    ...
    <div class="panel wide staff-users-panel"><h2>کارکنان تعریف‌شده</h2><p>این لیست فقط کارکنانی را نشان می‌دهد که مالک تعریف کرده است؛ حساب مالک/خریدار پکیج اینجا ثبت نمی‌شود. مدیر پکیج می‌تواند نام، کد پرسنلی، پین جدید، نقش، وضعیت و حذف هر کارمند را مدیریت کند.</p><div class="staff-user-list">${staffUsers.map(renderStaffUserManagementRow).join('') || '<p>هنوز کارمندی تعریف نشده است.</p>'}</div></div>
    <div class="panel wide security-events-panel"><h2>خط زمانی رویدادهای امنیتی</h2><p>مدیر می‌تواند دعوت کارکنان، لغو دعوت، پذیرش دعوت، درخواست بازیابی رمز، استفاده از کد بازیابی و تغییر وضعیت کارکنان را برای بررسی دسترسی‌های مشکوک ببیند.</p><form id="securityEventFilterForm" class="filter-form"><label>نوع رویداد<select name="type"><option value="" ${selected('', securityEventFilter.type)}>همه رویدادهای امنیتی</option>${securityTypes.map(type=>`<option value="${type}" ${selected(type, securityEventFilter.type)}>${esc(securityEventTypeLabel(type))}</option>`).join('')}</select></label><label>بازه زمانی<select name="range"><option value="all" ${selected('all', securityEventFilter.range)}>همه زمان‌ها</option><option value="today" ${selected('today', securityEventFilter.range)}>امروز</option><option value="seven" ${selected('seven', securityEventFilter.range)}>هفت روز اخیر</option><option value="thirty" ${selected('thirty', securityEventFilter.range)}>سی روز اخیر</option></select></label><button class="secondary">اعمال فیلتر رویداد امنیتی</button><button type="button" class="secondary" data-export-security-events>دریافت فایل رویدادهای امنیتی</button></form>${securityEvents.map(event=>`<div class="security-event-row"><b>${esc(event.title)}</b><span>${esc([event.targetName, event.targetEmail, event.detail].filter(Boolean).join(' — '))}</span><em>${formatDate(event.createdAt)}</em></div>`).join('') || '<p>رویدادی با این فیلتر پیدا نشد.</p>'}</div>
    <div class="panel"><h2>پشتیبان‌گیری و بازیابی</h2><p>از همه داده‌های نمونه اولیه در همین مرورگر فایل پشتیبان بگیر؛ هنگام بازیابی، حساب‌ها، منوها، انبار، فروش‌ها، رویدادهای امنیتی و دفتر مالی با محتوای فایل جایگزین می‌شوند.</p><div class="button-row"><button type="button" class="secondary" id="backupExport">دریافت فایل پشتیبان</button><input id="backupImportInput" type="file" accept="application/json" hidden><button type="button" class="secondary" id="backupImport">بازیابی از فایل پشتیبان</button><button type="button" class="secondary" id="sampleDataExport">دریافت فایل داده نمونه</button><button type="button" class="danger-button" id="sampleDataReset">بازنشانی به داده نمونه پاک</button></div><small>بازنشانی، داده‌های فعلی همین مرورگر را با حساب نمونه پاک جایگزین می‌کند؛ قبل از آن فایل پشتیبان بگیرید.</small>${backupMessage ? `<p class="success-message">${esc(backupMessage)}</p>` : ''}</div>
    <div class="panel wide"><h2>مشخصات مشتری</h2><p><b>${esc(customer.businessName)}</b></p><p>مالک: ${esc(customer.ownerName)}</p><p>موبایل: ${esc(customer.phone)}</p><p>ایمیل: ${esc(customer.email)}</p><h3>ماژول‌های فعال</h3><div class="cards">${RestaurantCore.getEnabledModules(state, customer.id).map(m=>`<div><b>${esc(moduleLabel(m))}</b><span>فعال</span></div>`).join('')}</div></div>
    <div class="panel wide"><h2>نقشه مهاجرت به نسخه واقعی</h2><p>هدف فنی: اپلیکیشن سروری، پایگاه داده رابطه‌ای و ورود امن کارکنان، بدون تغییر رفتار تست‌شده نمونه اولیه.</p><div class="cards">${migrationPlan.tables.map(t=>`<div><b>${esc(migrationCollectionLabel(t.prototypeCollection))}</b><span>${esc(t.purpose)}${t.tenantScoped ? ' — جدا برای هر مشتری' : ' — مشترک سامانه'}</span></div>`).join('')}</div><h3>گام‌های اجرایی</h3>${migrationPlan.phases.map(p=>`<div class="order-row"><b>گام ${numberText(p.order,0)}: ${esc(p.title)}</b><span>${esc(p.exitCriteria)}</span></div>`).join('')}</div>
    <div class="panel wide auth-migration-panel"><h2>طرح ورود امن نسخه واقعی</h2><p>این طرح مسیر تبدیل ورود نمونه اولیه به نشست سروری، محافظت فرم‌ها، خروج قابل ابطال و محافظ مسیرها را مشخص می‌کند.</p><h3>فیلدهای جدول نشست‌ها</h3><div class="cards">${authPlan.sessionTableFields.map(field=>`<div><b>${esc(field.name)}</b><span>${esc(field.purpose)}</span></div>`).join('')}</div><h3>رفتار کوکی، توکن ضد جعل درخواست و خروج</h3>${authPlan.requestProtection.map((rule,index)=>`<div class="order-row"><b>${numberText(index + 1,0)}</b><span>${esc(rule)}</span></div>`).join('')}<h3>محافظ مسیرها</h3>${authPlan.routeGuards.map(guard=>`<div class="order-row"><b>${esc(guard.area)}</b><span>${esc(guard.rule)}</span></div>`).join('')}<h3>بازیابی رمز عبور</h3>${authPlan.passwordRecovery.map((rule,index)=>`<div class="order-row"><b>${numberText(index + 1,0)}</b><span>${esc(rule)}</span></div>`).join('')}<h3>دعوت کارکنان</h3>${authPlan.staffInvitation.map((rule,index)=>`<div class="order-row"><b>${numberText(index + 1,0)}</b><span>${esc(rule)}</span></div>`).join('')}</div>
  </section>`;
}
...
function bindCommon() {
  ...
  document.querySelector('#logout').addEventListener('click', () => { if (session?.id && RestaurantCore.logout) RestaurantCore.logout(state, session.id); setActiveSession(null); saveState(); render(); });
  ...
  const handlers = {
    ...
    hallSaleForm: (f, form) => {
      ...
      const order = RestaurantCore.createHallOrder(state, customer.id, tableId, lines, { orderNote: f.get('orderNote') || '' });
    ...
    staffForm: (f) => RestaurantCore.createStaffUser(state, customer.id, { name: cleanPersianText(f.get('name')), personnelCode: toEnglishDigits(f.get('personnelCode')), pin: toEnglishDigits(f.get('pin')), role: f.get('role') }),
