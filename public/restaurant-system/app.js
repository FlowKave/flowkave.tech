...
function renderStaffCreateModal(staffUsers) {
  ...
  return `<div class="personnel-modal-overlay" data-personnel-modal-overlay><form class="panel personnel-modal-card staff-create-modal" id="staffForm" role="dialog" aria-modal="true" aria-label="افزودن پرسنل"><button type="button" class="modal-close-icon" data-close-staff-modal aria-label="بستن">×</button><h2>افزودن پرسنل</h2><p>این فرم فقط پرونده پرسنلی می‌سازد؛ پین و دسترسی ورود در باکس جدا تعریف می‌شود.</p><div class="staff-profile-grid"><label>کد پرسنلی خودکار<input name="personnelCode" value="${esc(faNum(code))}" inputmode="numeric" dir="ltr" autocomplete="off" readonly data-auto-personnel-code></label><label>نام<input name="firstName" value=""></label><label>نام خانوادگی<input name="lastName" value=""></label><label>نام پدر<input name="fatherName" value=""></label><label>کد ملی<input name="nationalId" value="" inputmode="numeric" dir="ltr" data-national-id placeholder="۰۰۹-۶۵۷۸۴۳-۵"></label><label>شماره همراه<input name="mobile" value="" inputmode="tel" dir="ltr"></label><label><span class="field-label-inline">ایمیل <small class="field-optional">(اختیاری)</small></span><input name="email" value="" type="email" dir="ltr" autocomplete="email"></label><label>سمت شغلی<input name="jobTitle" value=""></label><label>حقوق هر ساعت${numInput('hourlyWage', '')}</label><label class="wide-field">آدرس محل سکونت<textarea name="address" rows="2"></textarea></label></div><button class="primary">ثبت پرونده پرسنلی</button></form></div>`;
}
...
function renderAttendanceManagementRow(row, staffUsers = [], schedules = []) {
  ...
  return `<tr class="attendance-row ${rowClass}" data-attendance-row="${esc(row.id)}"><td><b>${esc(staffName)}</b><small>${esc(faNum(staffUsers.find(u=>u.id===row.staffUserId)?.personnelCode || ''))}</small></td><td>${esc(faNum(dateTextToJalaliDate(row.date)))}</td><td>${esc(faNum(schedule?.startTime || row.scheduledStart || '--:--'))}</td><td>${esc(faNum(schedule?.endTime || row.scheduledEnd || '--:--'))}</td><td><input name="clockInTime" inputmode="numeric" data-shift-time value="${esc(faNum(attendanceClockValue(row, 'clockInAt')))}" placeholder="--:--"></td><td><input name="clockOutTime" inputmode="numeric" data-shift-time value="${esc(faNum(attendanceClockValue(row, 'clockOutAt')))}" placeholder="--:--"></td><td><div class="attendance-note-lines">${note || ''}</div></td><td><span class="badge">${attendanceStatusLabel(row)}</span></td><td><div class="staff-attendance-row-actions">${row.managerApproval === 'pending' ? `<button class="primary" type="button" data-save-attendance-row="${esc(row.id)}">تایید</button>` : ''}<button class="danger-button" type="button" data-delete-attendance="${esc(row.id)}">حذف</button></div></td></tr>`;
}
...
function bindCommon() {
  ...
  const handlers = {
    ...
    staffForm: (f) => { const staff = RestaurantCore.createStaffUser(state, customer.id, { personnelCode: toEnglishDigits(f.get('personnelCode') || nextPersonnelCode(state.staffUsers)), firstName: cleanPersianText(f.get('firstName')), lastName: cleanPersianText(f.get('lastName')), fatherName: cleanPersianText(f.get('fatherName')), nationalId: normalizeNationalIdForSave(f.get('nationalId')), mobile: toEnglishDigits(f.get('mobile')), email: f.get('email'), address: cleanPersianText(f.get('address')), jobTitle: cleanPersianText(f.get('jobTitle')), hourlyWage: parseFaNumber(f.get('hourlyWage')), role: 'cashier' }); staffFormModalOpen = false; return staff; },
