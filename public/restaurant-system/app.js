...
function render() {
  ...
  const navItems = [
    ['dashboard','داشبورد'],['customerBank','باشگاه مشتریان'],['aiAssistant','هوش مصنوعی'],['menu','منو'],['sales','صندوق'],['recipes','رسپی'],['inventory','انبار'],['accounting','حسابداری'],['personnel','پرسنل'],['account','تنظیمات']
...
function renderPersonnel(customer) {
  ...
  return `<section class="workspace personnel-workspace"><div class="panel wide personnel-hero"><div class="section-title"><h2>پرسنل و دسترسی‌ها</h2><span class="badge">پرونده پرسنلی + ورود به سامانه</span></div><p>هر نیرو اول یک پرونده پرسنلی دارد؛ اگر لازم بود برای همان پرونده روش ورود، نقش و سطح دسترسی سامانه هم فعال می‌شود. این ساختار برای حضور و غیاب و حقوق و دستمزد مرحله بعد آماده است.</p><div class="cards personnel-model-cards"><div><b>پرونده پرسنلی</b><span>نام، کد پرسنلی، سمت، وضعیت همکاری</span></div><div><b>دسترسی به سامانه</b><span>ورود با کد پرسنلی/پین یا دعوت ایمیلی برای نقش‌های حساس</span></div><div><b>حضور و حقوق</b><span>مسیر آماده برای ورود/خروج شیفت و محاسبه حقوق و دستمزد</span></div></div></div>
    <form class="panel" id="staffForm"><h2>افزودن پرسنل</h2><p>برای نیروی عملیاتی، کد پرسنلی و پین بسازید؛ برای نقش‌های حساس می‌توانید دعوت ایمیلی هم استفاده کنید. حساب مالک در لیست پرسنل نمایش داده نمی‌شود.</p><label>نام پرسنل<input name="name" value="صندوق‌دار شیفت"></label><label>کد پرسنلی<input name="personnelCode" value="۱۰۰۱" inputmode="numeric" dir="ltr" autocomplete="off"></label><label>پین ورود سریع<input name="pin" value="۱۲۳۴" type="password" inputmode="numeric"></label><label>نقش دسترسی<select name="role"><option value="cashier">صندوق‌دار</option><option value="manager">مدیر</option></select></label><button class="primary">افزودن پرسنل</button></form>
    ...
    <div class="panel wide staff-users-panel"><h2>لیست پرسنل فعال سامانه</h2><p>این لیست فقط نیروهایی را نشان می‌دهد که مالک تعریف کرده است؛ مدیر می‌تواند نام، کد پرسنلی، پین جدید، نقش دسترسی، وضعیت و حذف هر نفر را مدیریت کند.</p><div class="staff-user-list">${staffUsers.map(renderStaffUserManagementRow).join('') || '<p>هنوز پرسنلی تعریف نشده است.</p>'}</div></div>
  </section>`;
}
