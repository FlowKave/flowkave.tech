...
function bindCommon() {
  ...
  document.querySelectorAll('.staff-user-edit-form').forEach(form => form.addEventListener('submit', (e) => {
    ...
    try {
      RestaurantCore.updateStaffUser(state, customer.id, form.dataset.staffUserId, {
        ...
        mobile: toEnglishDigits(f.get('mobile')),
  ...
  const handlers = {
    ...
    staffForm: (f) => { const staff = RestaurantCore.createStaffUser(state, customer.id, { personnelCode: toEnglishDigits(f.get('personnelCode') || nextPersonnelCode(state.staffUsers)), firstName: cleanPersianText(f.get('firstName')), lastName: cleanPersianText(f.get('lastName')), fatherName: cleanPersianText(f.get('fatherName')), nationalId: normalizeNationalIdForSave(f.get('nationalId')), mobile: toEnglishDigits(f.get('mobile')), email: f.get('email'), address: cleanPersianText(f.get('address')), jobTitle: cleanPersianText(f.get('jobTitle')), hourlyWage: parseFaNumber(f.get('hourlyWage')), role: f.get('role') || 'cashier' }); staffFormModalOpen = false; return staff; },
